...
export default async function ProvidersPage({
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* ── Category filter (no "All", no emirates) — 6 per row, rounded rectangles ── */}
            ...
            {chipList.map(cat => {
              ...
              return (
                <Link key={cat} href={href}
                  className={`px-3 py-2 rounded-lg text-xs border transition-all flex items-center justify-between gap-1.5 ${
                    active
                      ? 'bg-indigo-600 text-white border-indigo-600 font-bold shadow-sm'
                      : 'text-gray-600 border-gray-200 hover:border-indigo-300 hover:text-indigo-600 hover:bg-indigo-50/40'
                  }`}>
                  <span className="truncate">{catLabel(cat, isAr)}</span>
                  <span className={`text-[10px] tabular-nums flex-shrink-0 ${active ? 'text-indigo-200' : 'text-gray-400'}`}>
