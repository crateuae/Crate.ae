...
export default function PackagingPage() {
  ...
  // Deep link from the Packaging hub: /packaging/planner?mode=cartons|repack|basket opens that tool directly.
  useEffect(()=>{
    const m = new URLSearchParams(window.location.search).get('mode')
    if (m==='cartons' || m==='repack' || m==='basket') {
      setMode(m)
      setTimeout(()=>{ calcSectionRef.current?.scrollIntoView({ behavior:'smooth', block:'start' }) }, 300)
    }
  },[])
