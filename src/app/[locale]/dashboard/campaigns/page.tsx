...
import {
  ...
  CheckCircle2, Mail, Store, Package, Pencil, Trash2, ExternalLink,
} from 'lucide-react'
      ...
      <div className="mt-6 bg-white border border-slate-200 rounded-2xl p-5">
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {campaigns.map(c => {
              const pending = Math.max(0, c.total_recipients - c.sent_count)
              return (
              <div key={c.id} className="flex items-center justify-between gap-3 border-b border-slate-50 last:border-0 py-2">
                <div className="min-w-0 flex-1">
                  <Link href={`/${locale}/dashboard/campaigns/${c.id}`}
                    className="font-bold text-sm text-slate-900 hover:text-violet-700 flex items-center gap-1 truncate group">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <span className={`text-[10px] px-2 py-0.5 rounded-lg border font-bold ${
                    ...
                    {(c.status === 'sent' || c.status === 'sending')
                      ? <span>{isAr ? 'أُرسل' : 'sent'} <span className="font-black">{c.sent_count}/{c.total_recipients}</span>{pending > 0 ? <span className="ms-1 text-amber-600">+{pending}</span> : ''}</span>
                      : c.status}
                  </span>
                  ...
                  {c.status === 'draft' ? (
                    ...
                    /* Sent campaigns: quick-link to detail page for add/resend */
                    <Link href={`/${locale}/dashboard/campaigns/${c.id}`}
                      className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-slate-100 text-slate-600 text-xs font-bold hover:bg-violet-100 hover:text-violet-700">
                      <Users className="w-3 h-3" />{isAr ? 'إدارة' : 'Manage'}
                    </Link>
                  )}
                </div>
              </div>
